...
monthsBetween dA dB =
    ...
    case (month dA, month dB) of
        ...
        (Just _, Just _) ->
            let
                startMonth = Maybe.withDefault Jan (month start)
                endMonth = Maybe.withDefault Jan (month end)
            in
            Just <| 12 * (year end - year start) + (monthToInt endMonth - monthToInt startMonth)

    --Debug.todo "Implement Date.monthsBetween"

sortDates: Date -> Date -> (Date, Date)
sortDates first second =
...
compare (Date d1) (Date d2) =
    case (d1.month, d2.month) of
        ...
        (_, _) ->
            let
                monthComparison = Basics.compare (monthToInt <| Maybe.withDefault Jan d1.month) (monthToInt <| Maybe.withDefault Jan d2.month)
                yearComparison = Basics.compare d1.year d2.year
            in
            chainCompare monthComparison yearComparison
...
dateToString (Date d) = 
    "Year: " ++ String.fromInt d.year ++ " " ++ 
                Maybe.withDefault "" (Maybe.map (\x -> "Month: " ++ monthToString x) d.month)
