...
compare (Interval intA) (Interval intB) =
    ...
    in
    chainCompare endOrder startOrder
...
view (Interval interval) =
    let
        ...
        startDateAttributes = [class "interval-start"]
        endDateAttributes = [class "interval-end"]

        startDateNode = p startDateAttributes [text <| dateToString interval.start]
        endDateNode =
            case interval.end of
                Nothing -> p endDateAttributes [text "Present"]
                Just endDate -> p endDateAttributes [text <| dateToString endDate]

        intervalLength =
            length (Interval interval)
                |> Maybe.map (\(years, months) -> "Years: " ++ String.fromInt years ++ ", Months: " ++ String.fromInt months)
                |> Maybe.map (\lengthText -> p [class "interval-length"] [text lengthText])

        childElements =
            [startDateNode, endDateNode] ++ Maybe.withDefault [] (Maybe.map (\x -> [ x ]) intervalLength)
    in
