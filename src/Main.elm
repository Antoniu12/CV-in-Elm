...
update msg model =
    case msg of
        ...
        SelectEventCategory category ->
            let
                updatedCategories = EventCategory.set category True model.selectedEventCategories
             in
                ({ model | selectedEventCategories = updatedCategories }, Cmd.none)

        DeselectEventCategory category ->
            let
                updatedCategories = EventCategory.set category False model.selectedEventCategories
            in
                ({ model | selectedEventCategories = updatedCategories }, Cmd.none)
...
getRepos: Cmd Msg
getRepos =  Http.get 
     { url = "https://api.github.com/users/Antoniu12/repos"
